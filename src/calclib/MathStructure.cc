			...
			switch(mstruct.type()) {
				...
				case STRUCT_POWER: {
					...
					if(mstruct[1].hasNegativeSign()) {
						...
						for(unsigned int i = 0; i < SIZE; i++) {
							ret = CHILD(i).merge_multiplication(mstruct, eo);
							if(ret == 0) {
								MathStructure mstruct2(mstruct);
								ret = mstruct2.merge_multiplication(CHILD(i), eo);
								if(ret == 1) {
									CHILD(i) = mstruct2;
								}
							}
