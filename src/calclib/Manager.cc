...
bool Manager::add(const Manager *mngr, MathOperation op, bool translate_) {
	if(!mngr) return true;
//	printf("[%s] %c [%s] (%i)\n", print(NUMBER_FORMAT_NORMAL, DISPLAY_FORMAT_FRACTION | DISPLAY_FORMAT_SCIENTIFIC).c_str(), op2ch(op), mngr->print(NUMBER_FORMAT_NORMAL, DISPLAY_FORMAT_FRACTION).c_str(), translate_);
	if(!mngr->isPrecise()) setPrecise(false);
...
//			return false;
	...
	if(op == OPERATION_ADD) {
			...
			case POWER_MANAGER: {
				switch(mngr->type()) {
					...
					case POWER_MANAGER: {
						...
						} else if(mngrs[0]->isNumber() && mngr->mngrs[0]->isNumber() && mngr->mngrs[1]->equals(mngrs[1])) {
							mngrs[0]->add(mngr->mngrs[0], OPERATION_MULTIPLY);
						} else {
							if(!translate_) {
								return false;
							}
		...
		bool b_trans = false;
		if((c_type == ADDITION_MANAGER || CALCULATOR->multipleRootsEnabled()) && mngr->isNumber() && !mngr->number()->isInteger() && !mngr->number()->isComplex()) {
			Number *num = mngr->number()->numerator();
			if(!num->isOne() && (CALCULATOR->multipleRootsEnabled() || !num->isMinusOne())) {
				Manager mngr2(num);
				bool b = add(&mngr2, op, translate_);
				Number *den = mngr->number()->denominator();
				den->recip();
				mngr2.set(den);
				if(!b) {
		...
		switch(c_type) {
			...
			case POWER_MANAGER: {
				if(!CALCULATOR->multipleRootsEnabled() || (mngr->isNumber() && mngr->number()->isMinusOne()) || (mngrs[1]->isNumber() && mngrs[1]->number()->isInteger() && mngr->isNumber() && mngr->number()->isInteger())) {
					if(mngrs[0]->add(mngr, OPERATION_RAISE, false)) {
		...
		if(b_trans) {
			if(!CALCULATOR->multipleRootsEnabled() && mngr->isMultiplication()) {
				...
				for(i = 0; i < mngr->mngrs.size(); i++) {
					if(add(mngr->mngrs[i], OPERATION_RAISE, false)) {
						b = true;
						break;
					}
				}
				if(b) {
					sort();
					if(mngr->mngrs.size() == 2) {
						if(i == 0) {
							add(mngr->mngrs[1], OPERATION_RAISE);
						} else {
							add(mngr->mngrs[0], OPERATION_RAISE);
						}
					} else if(mngr->mngrs.size() > 2) {
						Manager mngr2;
						mngr2.setType(MULTIPLICATION_MANAGER);
						for(unsigned int i2 = 0; i2 < mngr->mngrs.size(); i2++) {
							if(i2 != i) {
								mngr->mngrs[i2]->ref();
								mngr2.push_back(mngr->mngrs[i2]);
							}
						}
				...
				if(b) {
					...
					if(mngr->mngrs.size() == 2) {
						if(i == 0) {
							mngr3.add(mngr->mngrs[1], OPERATION_RAISE);
						} else {
							mngr3.add(mngr->mngrs[0], OPERATION_RAISE);
						}
					} else if(mngr->mngrs.size() > 2) {
						Manager mngr2;
						mngr2.setType(ADDITION_MANAGER);
						for(unsigned int i2 = 0; i2 < mngr->mngrs.size(); i2++) {
							if(i2 != i) {
								mngr->mngrs[i2]->ref();
								mngr2.push_back(mngr->mngrs[i2]);
						...
						mngr3.add(&mngr2, OPERATION_RAISE);
					} else {
						mngr3.set(1, 1);
					}
					add(&mngr3, OPERATION_MULTIPLY);
					b_trans = false;
				}
			}
			if(b_trans) {
	...
	if(c_type == mngr->type()) {
		if(c_type == MULTIPLICATION_MANAGER) {
			if(mngrs[0]->isNumber() && !mngr->mngrs[0]->isNumber()) {
				...
				for(unsigned int i = 1; i < mngrs.size(); i++) {
					if(!mngrs[i]->equals(mngr->mngrs[i - 1])) return false;
				}
				...
				for(unsigned int i = 1; i < mngr->mngrs.size(); i++) {
					if(!mngrs[i - 1]->equals(mngr->mngrs[i])) return false;
				}
			} else if(mngrs[0]->isNumber() && mngr->mngrs[0]->isNumber()) {	
				if(mngrs.size() != mngr->mngrs.size()) return false;
				for(unsigned int i = 1; i < mngrs.size(); i++) {
...
bool Manager::convert(string unit_str) {
	Manager *mngr = CALCULATOR->calculate_sub(unit_str);
	bool b = convert(mngr);
	mngr->unref();
	return b;
}
...
/*			} else if(function() == CALCULATOR->getSinFunction() && mngrs.size() == 1) {
			...
			} else if(function() == CALCULATOR->getDiffFunction() && mngrs.size() == 2 && mngrs[1]->text() == x_var) {
				Manager *mngr = new Manager(2, 1);
				push_back(mngr);
			} else if(function() == CALCULATOR->getDiffFunction() && mngrs.size() == 1 && x_var == "x") {
	...
	switch(c_type) {
		...
		case MULTIPLICATION_MANAGER: {
			...
			if(!contains(&mngr2)) {			
				...
				Manager mngr_nox, mngr_x;
				mngr_nox.setType(MULTIPLICATION_MANAGER);
				mngr_x.setType(MULTIPLICATION_MANAGER);
				for(unsigned int i = 0; i < mngrs.size(); i++) {
					if(mngrs[i]->contains(&mngr2)) {
						mngr_x.push_back(new Manager(mngrs[i]));	
					} else {
						mngr_nox.push_back(new Manager(mngrs[i]));
					}
				}
				mngr_x.clean();
				mngr_nox.clean();
				if(mngr_nox.isNull()) {
					...
					mngr_x.integrate(x_var);
					set(&mngr_nox);
					add(&mngr_x, OPERATION_MULTIPLY);
				}
			}
			break;
		}
		case STRING_MANAGER: {
			if(s_var == x_var) {
				addInteger(2, OPERATION_RAISE);
				addInteger(2, OPERATION_DIVIDE);
			} else {
				Manager mngr(x_var);
				add(&mngr, OPERATION_MULTIPLY);
			}
	...
	switch(type()) {
		...
		case ADDITION_MANAGER: {
			...
			for(unsigned int i = 0; i < mngrs.size(); i++) {
				...
				str += mngrs[i]->print(nrformat, displayflags, min_decimals, max_decimals, in_exact, usable, set_prefix, false, NULL, l_exp, in_composite, in_power, false, print_equals, false, mngrs[i]->isAddition() || mngrs[i]->isComparison());
			}
...
//			printf("_IDNT: %s\n", giac_gen.print().c_str());
			if(giac_gen._IDNTptr->name->length() > 6 && giac_gen._IDNTptr->name->substr(0, 6) == "_unit_") {
				...
				Manager *mngr = CALCULATOR->calculate_sub(giac_gen._IDNTptr->name->substr(10, giac_gen._IDNTptr->name->length() - 10));
...
string Manager::find_x_var() {
	if(isText()) return s_var;
	string str, str2;
	for(unsigned int i = 0; i < mngrs.size(); i++) {
		str2 = mngrs[i]->find_x_var();
		if(str2 == "x") {
			return str2;
		} else if(str.empty()) {
			str = str2;
		} else if(str2.length() == 1) {
			if(str.length() > 1 || str2[0] == 'y' || ((str2[0] == 'z' || str2[0] < str[0]) && str[0] != 'y' && str[0] != 'z')) {
				str = str2;
			}
		}
	}
...
void Manager::move_x_to_one_side(string x_var) {
	...
	if(mngrs[0]->isAddition()) {
		...
		for(unsigned int i = 0; i < mngrs[0]->countChilds(); i++) {
			if(!mngrs[0]->getChild(i)->contains(&mngr_x)) {
				mngrs[1]->add(mngrs[0]->getChild(i), OPERATION_SUBTRACT);
				b = true;
			} else {
				mngr_new.add(mngrs[0]->getChild(i), OPERATION_ADD);
			}
		}
		if(mngr_new.isAddition()) {
			...
			if(!multiple && degree >= 0) {
				...
				fac_mngr1.addInteger(-1, OPERATION_MULTIPLY);
	...
	} else if(mngrs[0]->isMultiplication()) {
		Manager mngr_x(x_var);
		Manager mngr_new(1, 1);
		bool b = false;
		for(unsigned int i = 0; i < mngrs[0]->countChilds(); i++) {
			if(!mngrs[0]->getChild(i)->contains(&mngr_x)) {
		...
		if(mngrs[0]->base()->contains(&mngr_x)) {
			if(!mngrs[0]->exponent()->contains(&mngr_x)) {
				bool multiple = false;
				if(!CALCULATOR->multipleRootsEnabled() && mngrs[0]->exponent()->isNumber() && !mngrs[0]->exponent()->number()->isComplex()) {
					if(mngrs[0]->exponent()->number()->isInteger()) {
						if(mngrs[0]->exponent()->number()->isEven()) {	
							multiple = true;	
						}
					} else {
						Number *exp_num = mngrs[0]->exponent()->number()->numerator();
						if(exp_num->isEven()) {
							multiple = true;
						}
						delete exp_num;
...
void Manager::solve(string x_var) {
	if(!isComparison()) return;
	if((mngrs[0]->isText() && mngrs[0]->text() == x_var) || find_x_var() == x_var) return;
	mngrs[0]->add(mngrs[1], OPERATION_SUBTRACT);
	mngrs[1]->clear();
	move_x_to_one_side(x_var);
}
